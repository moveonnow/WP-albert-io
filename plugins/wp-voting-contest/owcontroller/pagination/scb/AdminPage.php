...
//  ____________MAIN METHODS____________
	...
	function admin_msg( $msg = '', $class = 'updated' ) {
		if ( empty( $msg ) )
			$msg = __( 'Settings <strong>saved</strong>.', 'voting-contest' );
...
//  ____________UTILITIES____________
	 ...
	 * @return string
	 */
	function submit_button( $value = '', $action = 'submit', $class = 'button-primary' ) {
		...
		return get_submit_button( __('Save Changes','voting-contest'), $args['class'], $args['action'] );
	}
...
//  ____________INTERNAL METHODS____________
	...
	function page_init() {
		extract( $this->args );
		if ( ! $toplevel ) {
			$this->pagehook = add_submenu_page( VOTES_TYPE_PAGIN, $page_title, '<span class="paginate_admin_menu_vote">'.$menu_title.'</span>', $capability, $page_slug, array( $this, '_page_content_hook' ) );
		} else {
	...
	private function check_args() {
		...
		$this->args = wp_parse_args( $this->args, array(
			...
			'icon_url'              => '',
			'screen_icon'           => '',
			'parent' => 'edit.php?post_type=' . OW_VOTES_TYPE,
			'capability'            => 'manage_options',
			'menu_title'            => $this->args['page_title'],
			'page_slug'             => '',
			'nonce'                 => '',
			'action_link'           => __( 'Settings', 'voting-contest' ),
			'ajax_submit' => false,
			'admin_action_priority' => 20,
		) );
