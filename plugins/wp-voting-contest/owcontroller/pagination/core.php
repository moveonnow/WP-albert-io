 ...
 */
function voting_wp_pagenavi( $args = array(),$term_ids = NULL) {
	if ( !is_array( $args ) ) {
		$argv = func_get_args();
	...
	$options = wp_parse_args( $options, Voting_PageNavi_Core::$options->get() );

	$instance = new Voting_PageNavi_Call( $args );
	...
	switch ( intval( $options['style'] ) ) {
		...
		case 2:
			...
			$out .= 'Go to Page:<select size="1" class="'.$term_ids.'">'."\n";
	...
	$out = $before . "<div class='wp-pagenavi ow_votes-pagination' id='".$term_ids."' >\n$out\n</div>" . $after;

        return $out;
	/*$out = apply_filters( 'voting_wp_pagenavi', $out );
...
class Voting_PageNavi_Call {
	...
	function get_url( $page ) {
		return ( 'multipart' == $this->type ) ? get_multipage_link( $page ) : vote_get_pagenum_link( $page );
	}
}
...
if ( !function_exists( 'vote_get_pagenum_link' ) ) :
function vote_get_pagenum_link($pagenum = 1, $escape = true ) {
	global $wp_rewrite;

	$pagenum = (int) $pagenum;

	$check_front_page = is_front_page();
	 if($check_front_page){
	     $frontpage_id = get_option('page_on_front');
             $struct_perma = get_option('permalink_structure');
             if($struct_perma=='')
	     $request = '?page_id='.$frontpage_id;
             else
...
function voting_wp_pagenavi_dropdown() {
	voting_wp_pagenavi();
}


class Voting_PageNavi_Core {
	static $options;

	static function init( $options ) {
		self::$options = $options;
                
                //Commented to stop default initialisation
