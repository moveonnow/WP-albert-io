...
/**Ohio webtech***/
	...
	include(OW_CONTROLLER_XL_PATH.'ow_vote_excel_reader.php');
	class Ow_Vote_SpreadsheetReader implements Iterator, Countable
	{
		...
		public function __construct($Filepath, $OriginalFilename = false, $MimeType = false)
		{
			...
			{
				throw new Exception('Ow_Vote_SpreadsheetReader: File ('.$Filepath.') not readable');
			}
			...
			if ($this -> Type == self::TYPE_XLS)
			{
				...
				$this -> Handle = new Ow_Vote_SpreadsheetReader_XLS($Filepath);
			...
			switch ($this -> Type)
			{
				case self::TYPE_XLSX:
					...
					$this -> Handle = new Ow_Vote_SpreadsheetReader_XLSX($Filepath);
					break;
				case self::TYPE_CSV:
					self::Load(self::TYPE_CSV);
					$this -> Handle = new Ow_Vote_SpreadsheetReader_CSV($Filepath, $this -> Options);
					break;
				case self::TYPE_XLS:
				...
				case self::TYPE_ODS:
					...
					$this -> Handle = new Ow_Vote_SpreadsheetReader_ODS($Filepath, $this -> Options);
					break;
			}
		}
		...
		private static function Load($Type)
		{
			...
			{
				throw new Exception('Ow_Vote_SpreadsheetReader: Invalid type ('.$Type.')');
			}

			if (!class_exists('Ow_Vote_SpreadsheetReader_'.$Type))
			{
				require(dirname(__FILE__).DIRECTORY_SEPARATOR.'ow_vote_spreadsheetreader_'.$Type.'.php');
			}
		}
