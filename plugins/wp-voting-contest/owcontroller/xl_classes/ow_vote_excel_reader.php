...
function ow_vote_GetInt4d($data, $pos) {
...
function ow_vote_gmgetdate($ts = null){
	$k = array('seconds','minutes','hours','mday','wday','mon','year','yday','weekday','month',0);
	return(ow_vote_array_comb($k,explode(":",gmdate('s:i:G:j:w:n:Y:z:l:F:U',is_null($ts)?time():$ts))));
	} 

// Added for PHP4 compatibility
function ow_vote_array_comb($array1, $array2) {
	$out = array();
...
class Ow_Vote_OLERead {
	var $data = '';
	function Ow_Vote_OLERead(){	}

	function ow_vote_read($sFileName){
		...
		$this->numBigBlockDepotBlocks = ow_vote_GetInt4d($this->data, OW_VOTE_NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
		$this->sbdStartBlock = ow_vote_GetInt4d($this->data, OW_VOTE_SMALL_BLOCK_DEPOT_BLOCK_POS);
		$this->rootStartBlock = ow_vote_GetInt4d($this->data, OW_VOTE_ROOT_START_BLOCK_POS);
		$this->extensionBlock = ow_vote_GetInt4d($this->data, OW_VOTE_EXTENSION_BLOCK_POS);
		$this->numExtensionBlocks = ow_vote_GetInt4d($this->data, OW_VOTE_NUM_EXTENSION_BLOCK_POS);
		...
		for ($i = 0; $i < $bbdBlocks; $i++) {
			$bigBlockDepotBlocks[$i] = ow_vote_GetInt4d($this->data, $pos);
			$pos += 4;
		}
		...
		for ($j = 0; $j < $this->numExtensionBlocks; $j++) {
			...
			if ($bbdBlocks < $this->numBigBlockDepotBlocks) {
				$this->extensionBlock = ow_vote_GetInt4d($this->data, $pos);
			}
		}
		...
		for ($i = 0; $i < $this->numBigBlockDepotBlocks; $i++) {
			...
			for ($j = 0 ; $j < OW_VOTE_BIG_BLOCK_SIZE / 4; $j++) {
				$this->bigBlockChain[$index] = ow_vote_GetInt4d($this->data, $pos);
				$pos += 4 ;
				$index++;
			}
		}
		...
		while ($sbdBlock != -2) {
		  ...
		  for ($j = 0; $j < OW_VOTE_BIG_BLOCK_SIZE / 4; $j++) {
			$this->smallBlockChain[$index] = ow_vote_GetInt4d($this->data, $pos);
			$pos += 4;
			$index++;
		  }
		...
		$pos = 0;
		$this->entry = $this->__ow_vote_readData($block);
		$this->__ow_vote_readPropertySets();
	}

	function __ow_vote_readData($bl) {
		$block = $bl;
		$pos = 0;
		$data = '';
		while ($block != -2)  {
	...
	function __ow_vote_readPropertySets(){
		$offset = 0;
		while ($offset < strlen($this->entry)) {
			...
			$startBlock = ow_vote_GetInt4d($d, OW_VOTE_START_BLOCK_POS);
			$size = ow_vote_GetInt4d($d, OW_VOTE_SIZE_POS);
			$name = '';
	...
	function ow_vote_getWorkBook(){
		if ($this->props[$this->wrkbook]['size'] < OW_VOTE_SMALL_BLOCK_THRESHOLD){
			$rootdata = $this->__ow_vote_readData($this->props[$this->rootentry]['startBlock']);
			$streamData = '';
...
* Main Class
*/
class Ow_Vote_Spreadsheet_Excel_Reader {
	...
	function ow_vote_myHex($d) {
		if ($d < 16) return "0" . dechex($d);
		return dechex($d);
	}
	
	function ow_vote_dumpHexData($data, $pos, $length) {
		$info = "";
		for ($i = 0; $i <= $length; $i++) {
			$info .= ($i==0?"":" ") . $this->ow_vote_myHex(ord($data[$pos + $i])) . (ord($data[$pos + $i])>31? "[" . $data[$pos + $i] . "]":'');
		}
		return $info;
	}

	function ow_vote_getCol($col) {
		if (is_string($col)) {
			$col = strtolower($col);
	...
	function ow_vote_val($row,$col,$sheet=0) {
		$col = $this->ow_vote_getCol($col);
	...
	function ow_vote_value($row,$col,$sheet=0) {
		return $this->ow_vote_val($row,$col,$sheet);
	}
	function ow_vote_info($row,$col,$type='',$sheet=0) {
		$col = $this->ow_vote_getCol($col);
	...
	function ow_vote_type($row,$col,$sheet=0) {
		return $this->ow_vote_info($row,$col,'type',$sheet);
	}
	function ow_vote_raw($row,$col,$sheet=0) {
		return $this->ow_vote_info($row,$col,'raw',$sheet);
	}
	function ow_vote_rowspan($row,$col,$sheet=0) {
		$val = $this->ow_vote_info($row,$col,'rowspan',$sheet);
		if ($val=="") { return 1; }
		return $val;
	}
	function ow_vote_colspan($row,$col,$sheet=0) {
		$val = $this->ow_vote_info($row,$col,'colspan',$sheet);
		if ($val=="") { return 1; }
		return $val;
	}
	function ow_vote_hyperlink($row,$col,$sheet=0) {
	...
	function ow_vote_rowcount($sheet=0) {
		return $this->sheets[$sheet]['numRows'];
	}
	function ow_vote_colcount($sheet=0) {
		return $this->sheets[$sheet]['numCols'];
	}
	function ow_vote_colwidth($col,$sheet=0) {
	...
	function ow_vote_colhidden($col,$sheet=0) {
		return !!$this->colInfo[$sheet][$col]['hidden'];
	}
	function ow_vote_rowheight($row,$sheet=0) {
		return $this->rowInfo[$sheet][$row]['height'];
	}
	function ow_vote_rowhidden($row,$sheet=0) {
		return !!$this->rowInfo[$sheet][$row]['hidden'];
	}
	
	// GET THE CSS FOR FORMATTING
	// ==========================
	function ow_vote_style($row,$col,$sheet=0,$properties='') {
		$css = "";
		$font=$this->ow_vote_font($row,$col,$sheet);
		if ($font!="") {
			$css .= "font-family:$font;";
		}
		$align=$this->ow_vote_align($row,$col,$sheet);
		if ($align!="") {
			$css .= "text-align:$align;";
		}
		$height=$this->ow_vote_height($row,$col,$sheet);
		if ($height!="") {
			$css .= "font-size:$height"."px;";
		}
		$bgcolor=$this->ow_vote_bgColor($row,$col,$sheet);
		if ($bgcolor!="") {
			$bgcolor = $this->colors[$bgcolor];
			$css .= "background-color:$bgcolor;";
		}
		$color=$this->ow_vote_color($row,$col,$sheet);
		if ($color!="") {
			$css .= "color:$color;";
		}
		$bold=$this->ow_vote_bold($row,$col,$sheet);
		if ($bold) {
			$css .= "font-weight:bold;";
		}
		$italic=$this->ow_vote_italic($row,$col,$sheet);
		if ($italic) {
			$css .= "font-style:italic;";
		}
		$underline=$this->ow_vote_underline($row,$col,$sheet);
		if ($underline) {
			$css .= "text-decoration:underline;";
		}
		// Borders
		$bLeft = $this->ow_vote_borderLeft($row,$col,$sheet);
		$bRight = $this->ow_vote_borderRight($row,$col,$sheet);
		$bTop = $this->ow_vote_borderTop($row,$col,$sheet);
		$bBottom = $this->ow_vote_borderBottom($row,$col,$sheet);
		$bLeftCol = $this->ow_vote_borderLeftColor($row,$col,$sheet);
		$bRightCol = $this->ow_vote_borderRightColor($row,$col,$sheet);
		$bTopCol = $this->ow_vote_borderTopColor($row,$col,$sheet);
		$bBottomCol = $this->ow_vote_borderBottomColor($row,$col,$sheet);
	...
	// FORMAT PROPERTIES
	// =================
	function ow_vote_format($row,$col,$sheet=0) {
		return $this->ow_vote_info($row,$col,'format',$sheet);
	}
	function ow_vote_formatIndex($row,$col,$sheet=0) {
		return $this->ow_vote_info($row,$col,'formatIndex',$sheet);
	}
	function ow_vote_formatColor($row,$col,$sheet=0) {
		return $this->ow_vote_info($row,$col,'formatColor',$sheet);
	}
	
	// CELL (XF) PROPERTIES
	// ====================
	function ow_vote_xfRecord($row,$col,$sheet=0) {
		$xfIndex = $this->ow_vote_info($row,$col,'xfIndex',$sheet);
		if ($xfIndex!="") {
			return $this->xfRecords[$xfIndex];
		}
		return null;
	}
	function ow_vote_xfProperty($row,$col,$sheet,$prop) {
		$xfRecord = $this->ow_vote_xfRecord($row,$col,$sheet);
		if ($xfRecord!=null) {
			return $xfRecord[$prop];
		}
		return "";
	}
	function ow_vote_align($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'align');
	}
	function ow_vote_bgColor($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'bgColor');
	}
	function ow_vote_borderLeft($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'borderLeft');
	}
	function ow_vote_borderRight($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'borderRight');
	}
	function ow_vote_borderTop($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'borderTop');
	}
	function ow_vote_borderBottom($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'borderBottom');
	}
	function ow_vote_borderLeftColor($row,$col,$sheet=0) {
		return $this->colors[$this->ow_vote_xfProperty($row,$col,$sheet,'borderLeftColor')];
	}
	function ow_vote_borderRightColor($row,$col,$sheet=0) {
		return $this->colors[$this->ow_vote_xfProperty($row,$col,$sheet,'borderRightColor')];
	}
	function ow_vote_borderTopColor($row,$col,$sheet=0) {
		return $this->colors[$this->ow_vote_xfProperty($row,$col,$sheet,'borderTopColor')];
	}
	function ow_vote_borderBottomColor($row,$col,$sheet=0) {
		return $this->colors[$this->ow_vote_xfProperty($row,$col,$sheet,'borderBottomColor')];
	}

	// FONT PROPERTIES
	// ===============
	function ow_vote_fontRecord($row,$col,$sheet=0) {
	    $xfRecord = $this->ow_vote_xfRecord($row,$col,$sheet);
		if ($xfRecord!=null) {
	...
	function ow_vote_fontProperty($row,$col,$sheet=0,$prop) {
		$font = $this->ow_vote_fontRecord($row,$col,$sheet);
		if ($font!=null) {
			return $font[$prop];
		}
		return false;
	}
	function ow_vote_fontIndex($row,$col,$sheet=0) {
		return $this->ow_vote_xfProperty($row,$col,$sheet,'fontIndex');
	}
	function ow_vote_color($row,$col,$sheet=0) {
		$formatColor = $this->ow_vote_formatColor($row,$col,$sheet);
		if ($formatColor!="") {
			return $formatColor;
		}
		$ci = $this->ow_vote_fontProperty($row,$col,$sheet,'color');
	...
	function ow_vote_bold($row,$col,$sheet=0) {
		return $this->ow_vote_fontProperty($row,$col,$sheet,'bold');
	}
	function ow_vote_italic($row,$col,$sheet=0) {
		return $this->ow_vote_fontProperty($row,$col,$sheet,'italic');
	}
	function ow_vote_underline($row,$col,$sheet=0) {
		return $this->ow_vote_fontProperty($row,$col,$sheet,'under');
	}
	function ow_vote_height($row,$col,$sheet=0) {
		return $this->ow_vote_fontProperty($row,$col,$sheet,'height');
	}
	function ow_vote_font($row,$col,$sheet=0) {
		return $this->ow_vote_fontProperty($row,$col,$sheet,'font');
	}
	
	// DUMP AN HTML TABLE OF THE ENTIRE XLS DATA
	// =========================================
	function ow_vote_dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		if ($col_letters) {
			...
			for($i=1;$i<=$this->ow_vote_colcount($sheet);$i++) {
				$style = "width:" . ($this->ow_vote_colwidth($i,$sheet)*1) . "px;";
				if ($this->ow_vote_colhidden($i,$sheet)) {
					$style .= "display:none;";
				}
		...
		$out .= "<tbody>\n";
		for($row=1;$row<=$this->ow_vote_rowcount($sheet);$row++) {
			$rowheight = $this->ow_vote_rowheight($row,$sheet);
			$style = "height:" . ($rowheight*(4/3)) . "px;";
			if ($this->ow_vote_rowhidden($row,$sheet)) {
				$style .= "display:none;";
			}
			$out .= "\n\t<tr style=\"$style\">";
			if ($row_numbers) {
			...
			for($col=1;$col<=$this->ow_vote_colcount($sheet);$col++) {
				// Account for Rowspans/Colspans
				$rowspan = $this->ow_vote_rowspan($row,$col,$sheet);
				$colspan = $this->ow_vote_colspan($row,$col,$sheet);
				for($i=0;$i<$rowspan;$i++) {
					for($j=0;$j<$colspan;$j++) {
						if ($i>0 || $j>0) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					$style = $this->ow_vote_style($row,$col,$sheet);
					if ($this->ow_vote_colhidden($col,$sheet)) {
						$style .= "display:none;";
					}
					$out .= "\n\t\t<td style=\"$style\"" . ($colspan > 1?" colspan=$colspan":"") . ($rowspan > 1?" rowspan=$rowspan":"") . ">";
					$val = $this->ow_vote_val($row,$col,$sheet);
					if ($val=='') { $val="&nbsp;"; }
					else { 
						$val = htmlentities($val); 
						$link = $this->ow_vote_hyperlink($row,$col,$sheet);
						if ($link!='') {
	...
	function ow_vote_read16bitstring($data, $start) {
		$len = 0;
	...
	function _ow_vote_format_value($format,$num,$f) {
		// 49==TEXT format
	 ...
	 */
	function Ow_Vote_Spreadsheet_Excel_Reader($file='',$store_extended_info=true,$outputEncoding='') {
		$this->_ole = new Ow_Vote_OLERead();
		$this->ow_vote_setUTFEncoder('iconv');
		if ($outputEncoding != '') { 
			$this->ow_vote_setOutputEncoding($outputEncoding);
		}
		for ($i=1; $i<245; $i++) {
	 ...
	 */
	function ow_vote_setOutputEncoding($encoding) {
	 ...
	 */
	function ow_vote_setUTFEncoder($encoder = 'iconv') {
	...
	function ow_vote_setRowColOffset($iOffset) {
	 ...
	 */
	function ow_vote_setDefaultFormat($sFormat) {
	 ...
	 */
	function ow_vote_setColumnFormat($column, $sFormat) {
	 ...
	 */
	function ow_vote_read($sFileName) {
		$res = $this->_ole->ow_vote_read($sFileName);
		...
		$this->data = $this->_ole->ow_vote_getWorkBook();
		$this->_wp_vote_parse();
	}
		...
		while ($code != OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_EOF) {
			switch ($code) {
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					$uniqueStrings = $this->_ow_vote_GetInt4d($data, $spos+4);
					$spos += 8;
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						if ($extendedString) {
							// Read in cchExtRst
							$extendedRunLength = $this->_ow_vote_GetInt4d($data, $spos);
							$spos += 4;
						}
						...
						$retstr = ($asciiEncoding) ? $retstr : $this->_ow_vote_encodeUTF16($retstr);
				...
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_FONT:
						...
						if ((ord($data[$pos+19]) & 1) == 0){
						    ...
						    $font =  $this->_ow_vote_encodeUTF16($font); 
						}
						$this->fontRecords[] = array(
								...
								'bold' => ($weight==700),
								'font' => $font,
								'raw' => $this->ow_vote_dumpHexData($data, $pos+3, $length)
								);
					    break;
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
				...
				case Ow_VOTE_SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
						$rec_offset = $this->_ow_vote_GetInt4d($data, $pos+4);
						...
						if ($version == OW_VOTE_SPREADSHEET_EXCEL_READER_BIFF8){
							...
							if ($chartype == 0){
								...
								$rec_name	= $this->_ow_vote_encodeUTF16(substr($data, $pos+12, $rec_length*2));
							}
	...
	function _wp_vote_parsesheet($spos) {
		...
		while($cont) {
			...
			switch ($code) {
				...
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_RK2:
					...
					$rknum = $this->_ow_vote_GetInt4d($data, $spos + 6);
					$numValue = $this->_ow_vote_GetIEEE754($rknum);
					$info = $this->_ow_vote_getCellDetails($spos,$numValue,$column);
					$this->ow_vote_addcell($row, $column, $info['string'],$info);
					break;
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
					$row		= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column	 = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$xfindex	= ord($data[$spos+4]) | ord($data[$spos+5])<<8;
					$index  = $this->_ow_vote_GetInt4d($data, $spos + 6);
					$this->ow_vote_addcell($row, $column, $this->sst[$index], array('xfIndex'=>$xfindex) );
					break;
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_MULRK:
					...
					$tmppos = $spos+4;
					for ($i = 0; $i < $columns; $i++) {
						$numValue = $this->_ow_vote_GetIEEE754($this->_ow_vote_GetInt4d($data, $tmppos + 2));
						$info = $this->_ow_vote_getCellDetails($tmppos-4,$numValue,$colFirst + $i + 1);
						$tmppos += 6;
						$this->ow_vote_addcell($row, $colFirst + $i, $info['string'], $info);
					}
					break;
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
					...
					if ($this->ow_vote_isDate($spos)) {
						$numValue = $tmp['double'];
					}
					else {
						$numValue = $this->ow_vote_createNumber($spos);
					}
					$info = $this->_ow_vote_getCellDetails($spos,$numValue,$column);
					$this->ow_vote_addcell($row, $column, $info['string'], $info);
					break;
				...
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
					...
					if ((ord($data[$spos+6])==0) && (ord($data[$spos+12])==255) && (ord($data[$spos+13])==255)) {
                        ...
                        if (ord($this->data[$spos+8])==1) {
                            $this->ow_vote_addcell($row, $column, "TRUE");
                        } else {
                            $this->ow_vote_addcell($row, $column, "FALSE");
                        }
						...
						$this->ow_vote_addcell($row, $column, '');
					} else {
				...
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_BOOLERR:
					...
					$this->ow_vote_addcell($row, $column, $string);
					break;
                case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_STRING:
					...
					if ($version == OW_VOTE_SPREADSHEET_EXCEL_READER_BIFF8){
						...
						if ($extendedString) {
							// Read in cchExtRst
							$extendedRunLength =$this->_ow_vote_GetInt4d($this->data, $xpos);
							$xpos += 4;
						}
						$len = ($asciiEncoding)?$numChars : $numChars*2;
					...
					$this->ow_vote_addcell($previousRow, $previousCol, $retstr);
					break;
				...
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_MULBLANK:
					...
					for ($c = 0; $c < $cols; $c++) {
						...
						$this->ow_vote_addcell($row, $column + $c, "", array('xfIndex'=>$xfindex));
					}
					break;
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_LABEL:
					$row	= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$this->ow_vote_addcell($row, $column, substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7])<<8));
					break;
				...
				case OW_VOTE_SPREADSHEET_EXCEL_READER_TYPE_HYPER:
					...
					if (($flags & 1) > 0 ) {   // is a type we understand
						...
						$ulink = $this->ow_vote_read16bitstring($this->data, $spos + $uloc + 20);
						if ($udesc == "") {
							$udesc = $ulink;
						}
					}
					$linkdata['desc'] = $udesc;
					$linkdata['link'] = $this->_ow_vote_encodeUTF16($ulink);
					for ($r=$row; $r<=$row2; $r++) { 
		...
		function _ow_vote_getCellDetails($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				$dateinfo = ow_vote_gmgetdate($utcValue);
				...
				$rectype = 'number';
				$formatted = $this->_ow_vote_format_value($format, $numValue, $formatIndex);
	...
	function ow_vote_createNumber($spos) {
		$rknumhigh = $this->_ow_vote_GetInt4d($this->data, $spos + 10);
		$rknumlow = $this->_ow_vote_GetInt4d($this->data, $spos + 6);
	...
	function ow_vote_addcell($row, $col, $string, $info=null) {
	...
	function _ow_vote_GetIEEE754($rknum) {
		if (($rknum & 0x02) != 0) {
				$value = $rknum >> 2;
		} else {
			//mmp
	...
	function _ow_vote_encodeUTF16($string) {
		$result = $string;
	...
	function _ow_vote_GetInt4d($data, $pos) {
		$value = ord($data[$pos]) | (ord($data[$pos+1]) << 8) | (ord($data[$pos+2]) << 16) | (ord($data[$pos+3]) << 24);
		if ($value>=4294967294) {
			$value=-2;
		}
