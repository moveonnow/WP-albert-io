...
if(!class_exists('Ow_Widget_Recent_Controller')){
	class Ow_Widget_Recent_Controller extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			if ( ! empty( $title ) )
				echo $args['before_title'] .'<span class="ow_recent_contest_h"><span class="ow_vote_icons voteconestant-camera" aria-hidden="true"></span>'. $title ."</span>".$args['after_title'];
			do_action('voting_display_recent_widget',$instance);      
...
die("<h2>".__('Failed to load the Voting Widget Recent Controller','voting-contest')."</h2>");
